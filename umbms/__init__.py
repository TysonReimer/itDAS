...
def verify_path(path):
    ...
    ----------
    path : str
        The path to investigate - NOTE: will only 'make it exist' if only the
        most-nested dir does not exist, but all parent directories do exist.
    """
...
def get_script_logger(script_path, level=logging.DEBUG):
    ...
    logger.info("UM-BMS Logger initialized in host [%s]." % (gethostname()))
